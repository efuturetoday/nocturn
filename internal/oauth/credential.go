...
// Credential is the host-side, refreshing credential. It wraps an oauth2.TokenSource
...
// NewCredential wraps cfg+initial into a refreshing Credential. onChange (may be nil) is
...
func NewCredential(cfg *oauth2.Config, initial *oauth2.Token, onChange func(*oauth2.Token)) *Credential {
...
// refresh — the token outlives any single request (see NewCredential).
func (s *Credential) Value(context.Context) ([]byte, error) {
	tok, err := s.ts.Token()
	if err != nil {
		return nil, err
	}
