;; echo.wat — a minimal WASI guest for internal/sandbox.
;; It reads stdin, sends it through the host function nocturn.echo using the
;; standard host↔wasm ABI (host mallocs the response in the guest, returns a
;; packed addr<<32|size), then writes the returned bytes to stdout. It exports
;; memory + a bump malloc/free so the host can allocate inside it.
;;
...
(module
  ...
  (func (export "_start")
    ...
    ;; host echo(inputPtr=1024, inputLen=nread) -> packed (addr<<32 | size)
    (local.set $packed (call $echo (i32.const 1024) (local.get $nread)))
