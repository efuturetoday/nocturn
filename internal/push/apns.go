...
// APNSConfig configures the Apple Push adapter. KeyPath points at the token-signing key (.p8,
// a PKCS#8 EC private key downloaded from the Apple Developer portal); KeyID and TeamID identify
// it; BundleID is the app's bundle identifier (the apns-topic). Production selects the live APNs
// host vs the sandbox. OnBadToken, if set, is called with any token APNs reports permanently
// invalid (410 / BadDeviceToken / Unregistered), so the caller can drop it.
type APNSConfig struct {
	...
	// set it to point at a mock gateway (tests) or a self-hosted relay.
	Host string
	// HTTPClient overrides the HTTP/2 client (tests inject one trusting a local TLS server). A
	// nil value uses the default client, which negotiates HTTP/2 over TLS via ALPN.
	HTTPClient *http.Client
	// now overrides the clock for the JWT iat (tests). Nil uses time.Now.
	now func() time.Time
}

// APNS delivers pushes to Apple Push Notification service over HTTP/2 with a token-based (ES256
// JWT) authorization. It implements Sender. The provider auth token is cached and reused up to
// APNs' one-hour ceiling (regenerated at ~50 minutes). Safe for concurrent use.
type APNS struct {
	cfg    APNSConfig
...
// NewAPNS loads and parses the .p8 signing key and builds the adapter. It fails if the key is
// missing, not PEM, or not an ECDSA PKCS#8 key.
func NewAPNS(cfg APNSConfig) (*APNS, error) {
	...
	if block == nil {
		return nil, fmt.Errorf("apns: %s is not PEM", cfg.KeyPath)
	}
...
// OnBadToken (and counts as neither delivered nor a hard error). It returns nil if at least one
// token was delivered, else an error — so a caller can treat "nobody reachable" as fail-closed.
...
func (a *APNS) push(ctx context.Context, jwt, token string, body []byte) error {
	...
	// APNs reports the reason as JSON; 410 (or a BadDeviceToken/Unregistered reason) means the
...
// payload wraps a Message in the APNs `aps` envelope plus any custom Data keys. Data is applied
// first so the reserved `aps` key can never be shadowed by a caller.
