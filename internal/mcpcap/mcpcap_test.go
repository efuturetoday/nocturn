...
func TestConn_CredentialInjection_OwnerScoped(t *testing.T) {
	...
	inj.AddBinding(owner, secret.Binding{Secret: own, Host: conn.Host(), Header: "Authorization", Prefix: "Bearer "})
	inj.AddBinding("plugin:x", secret.Binding{Secret: "plugin:x/oauth", Host: conn.Host(), Header: "X-Api-Key", Prefix: ""})
