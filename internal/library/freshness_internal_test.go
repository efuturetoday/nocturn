...
func TestFreshness_ARolledBackEntryIsRefused(t *testing.T) {
	...
	if got := validItems([]Item{current}, quiet(), signaturesRequired, seen); len(got) != 1 {
	...
	if got := validItems([]Item{old}, slog.New(slog.NewTextHandler(&log, nil)), signaturesRequired, seen); len(got) != 0 {
...
func TestFreshness_SurvivesAReopen(t *testing.T) {
	...
	validItems([]Item{serialItem(t, priv, 5)}, quiet(), signaturesRequired, first)

	if _, err := os.Stat(path); err != nil {
		t.Fatalf("nothing was recorded: %v", err)
	}
	reopened := openFreshness(path, nil)
	if got := validItems([]Item{serialItem(t, priv, 4)}, quiet(), signaturesRequired, reopened); len(got) != 0 {
		t.Error("a rollback was accepted after a restart")
	}
	if got := validItems([]Item{serialItem(t, priv, 6)}, quiet(), signaturesRequired, reopened); len(got) != 1 {
...
func TestFreshness_TheFirstSightIsAccepted(t *testing.T) {
	...
	if got := validItems([]Item{serialItem(t, priv, 1)}, quiet(), signaturesRequired, seen); len(got) != 1 {
...
func TestFreshness_ARefusedEntryDoesNotRaiseTheFloor(t *testing.T) {
	...
	seen := openFreshness(filepath.Join(t.TempDir(), serialFile), nil)
	if got := validItems([]Item{forged}, quiet(), signaturesRequired, seen); len(got) != 0 {
...
// installableManifest is what a plugin payload has to carry to get past checkPayloads — validItems
...
func serialItem(t *testing.T, priv ed25519.PrivateKey, serial int) Item {
	t.Helper()
	it := Item{
		ID: "gmail", Title: "Gmail", Description: "d", Serial: serial,
		PluginManifest: installableManifest, PluginScript: "// code",
	}
