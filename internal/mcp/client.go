// Package mcp is a minimal client for remote (HTTP) MCP servers — the SAFE half
// of MCP (ADR-9): a remote server runs no code on your machine, so talking to it
// is just gated HTTP. This package is protocol-only and stdlib-only: JSON-RPC
// 2.0 over an injected Transport, covering exactly the tools subset a CLIENT
// needs over the Streamable HTTP transport (spec revision 2025-11-25):
// initialize + notifications/initialized, tools/list (paginated), tools/call —
// with both response shapes the spec requires the client to support
// (application/json and text/event-stream). Gating (broker + HITL), credential
// injection, leak scanning, and the actual HTTP live in the Transport the caller
// supplies (internal/mcpcap) — so the client stays pure and testable, and every
// effect a tool call performs still crosses the gateway.
//
...
// message, so this bounds a message, mirroring netcap's response cap.
...
// and JSON Schema for arguments — mapped 1:1 to a nocturn tool.Spec by the caller.
