...
func TestExplain_NamesTheAuthCommand(t *testing.T) {
	p := &Plugin{manifest: Manifest{
		Name:  "gmail",
		Decl:  extension.Decl{Credentials: []CredentialDecl{{Name: "account", Host: "gmail.googleapis.com", Header: "Authorization"}}},
		OAuth: []OAuthDecl{{Name: "account"}},
	}}

	got := p.explain(fmt.Errorf("credential %q: %w", "ext:gmail@account", secret.ErrNotFound))
...
func TestExplain_NamesTheSecretForAHandSeededCredential(t *testing.T) {
	p := &Plugin{manifest: Manifest{
		Name: "tracker",
		Decl: extension.Decl{Credentials: []CredentialDecl{{Name: "token", Host: "api.example.com", Header: "Authorization"}}},
	}}

	got := p.explain(fmt.Errorf("credential: %w", secret.ErrNotFound)).Error()
	if !strings.Contains(got, "nocturn secret set tracker/token") {
