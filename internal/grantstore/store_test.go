...
func TestGrantsStore_PerOwnerIsolation(t *testing.T) {
	dir := t.TempDir()
	sessionStore := grantstore.Load(filepath.Join(dir, "grants.json"))
	agentStore := grantstore.Load(grantstore.Path(filepath.Join(dir, "agents"), "triage"))
...
func TestGrantsStore_PersistAndRevoke(t *testing.T) {
	...
	if fresh := grantstore.Load(path); fresh.Allows("gmail.send", call) {
