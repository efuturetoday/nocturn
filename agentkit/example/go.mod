module github.com/efuturetoday/nocturn/agentkit/example

go 1.26.1

require (
	github.com/efuturetoday/nocturn/agentkit v0.0.0
	github.com/efuturetoday/nocturn/agentkit/gate v0.0.0
	github.com/efuturetoday/nocturn/agentkit/openai v0.0.0
	github.com/efuturetoday/nocturn/agentkit/runtime v0.0.0
	github.com/efuturetoday/nocturn/agentkit/tools v0.0.0
)

require github.com/sashabaranov/go-openai v1.42.1 // indirect

replace github.com/efuturetoday/nocturn/agentkit => ..

replace github.com/efuturetoday/nocturn/agentkit/openai => ../openai

replace github.com/efuturetoday/nocturn/agentkit/gate => ../gate

replace github.com/efuturetoday/nocturn/agentkit/runtime => ../runtime

replace github.com/efuturetoday/nocturn/agentkit/tools => ../tools
