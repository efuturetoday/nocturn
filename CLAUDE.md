...
| `capability` | Reine Entscheidung (kein I/O, stdlib-only). Modell = **`Call{Capability, Target}`** × **`Rule{Capability, TargetGlob, …}`**: `Target` ist der **capability-definierte Resource-String** (host für http, Pfad für file, command für exec) — **nicht** HTTP-spezifisch (ehem. `Attrs{host}`/`HostGlob`, wie Claudes `Tool(specifier)`). Gematcht per `path.Match` (`*` kreuzt kein `/` → Pfad-Globs tiefenbegrenzt); **`Wildcard`** (`"*"`) ist der explizite Any-Token (matcht *jeden* Target inkl. Mehrsegment-Pfade, ein Target muss aber da sein). `Policy.Evaluate` (deny>ask>allow, fail-closed). `EpochRegistry`, `RateLimiter`, `Window`. Konstante `Wildcard`, `Permanent`. **`Ceiling`** = komponierbare Obergrenze (Schnittmenge, ctx-**Kette** `WithCeiling`/`CeilingsFrom`/`WithinCeilings`; leere Kette = vacuously true = fail-**open**, bewusst — s.u.). **`Grants`** (ehem. `Context`) = Eigentümer des stehenden Permission-Sets einer Session/Workspace: session-scoped (epoch-gebunden) + `always` via `GrantStore`-Interface (I/O-frei gehalten); `Allows`/`Record(call, scope)`; Scope-Konstanten **`ScopeOnce`/`ScopeSession`/`ScopeAlways`**; ctx-Seam `WithGrants`/`GrantsFrom`. |
...
Tools bilden Capabilities ab — `nocturn.call` ruft nie eine Capability direkt. Roadmap für semantisches
HITL-Wording + Consent am Tool-Boundary (statt Transport) steht in FRAGEN.md/als Follow-up.
