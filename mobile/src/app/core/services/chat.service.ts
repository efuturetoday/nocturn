...
export class ChatService {
  ...
  private buildSnapshotMessages(messages: Message[]): ChatMessageView[] {
    ...
    // into the current bubble, broken by a user message. This matches the LIVE render (one bubble per
    // turn); without it a reloaded turn splits the tool call from its answer into two bubbles.
    ...
    for (const m of messages) {
      ...
      if (current) {
        ...
        current.tools = [...current.tools, ...tools];
      } else {
